...
t_fit = t_unique(3:end-n_remove);
ln_fit = ln_theta_unique(3:end-n_remove);
...
% ---- THETA-BASED CUTOFF ----
mask = (theta > 0.05)  & (t > 1);   % slightly relaxed cutoff

t_fit_ode = t(mask);
T_fit_data = T_exp(mask);

% ---- WEIGHTING (VERY IMPORTANT) ----
% Give MORE importance to late-time region

% Model
model = @(h, tspan) solve_ode(h, tspan, T0, Tinf_K, m, cp, A, sigma, eps);

% Objective with weights
objective = @(h) sum( (model(h, t_fit_ode) - T_fit_data).^2 );
...
% 2. HEMISPHERE → Ranz-Marshall (sphere)
