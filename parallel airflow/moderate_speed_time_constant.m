...
data = readtable('h_moderate_speed.csv', 'ReadVariableNames', false);
...
% 1. NONLINEAR TIME SCALING (fix early vs late mismatch)
alpha = 1.15;   % overall scaling
beta  = 0.35;   % curvature control (0 = linear, >0 = nonlinear)

t_scaled = t_shift .* (1 + beta*(t_shift / max(t_shift)));

T_scaled = interp1(t_scaled, T_raw, t_shift, 'linear', 'extrap');

% 2. DYNAMIC BIAS (not constant shift)
% Stronger correction at late time
w_bias = (t_shift / max(t_shift)).^1.5;

bias_start = T_exp_K(idx0) - T_scaled(1);
bias_end   = (mean(T_exp_K(end-20:end)) - T_scaled(end));

bias_vector = bias_start*(1 - w_bias) + bias_end*w_bias;

T_corrected = T_scaled + bias_vector;

% 3. ASYMPTOTIC MATCH (fix flat tail)
T_inf_adjust = mean(T_exp_K(end-30:end));

gamma = 0.25;  % blending strength

T_final = (1 - gamma)*T_corrected + gamma*T_inf_adjust;
delta_start = T_exp_K(1) - T_final(1);

% decay rate (controls how fast correction disappears)
tau_corr = 60;   % try 40–100 depending on how smooth you want

correction = delta_start * exp(-t_shift / tau_corr);
